module ErrorSerializer
  ...
  def from_messages(status, error_messages, meta: {})
    error_messages = Array(error_messages)
    { status: status, errors: build_errors(error_messages, meta) }
  end
  alias from_message from_messages

  def from_model(status, model)
    { status: status, errors: build_model_errors(model.errors) }
  end
